...
#'      ylab = "y", xlab = "x")
#' filledLines(x, y1, y2, col = "tomato", border = NA)
#' filledLines(x, pmin(y1, y2), rep(-Inf, length(x)), 
...
#' filledLines(x, y, density = 10, angle = -30, col = "orange")
#' 
#' # The super egg
#' p <- 5/2
#' h <- 1
...
#'      xlab = "", ylab = "")
#' filledLines(x, y1, y2, col = "gold", lwd = 2, border = "orange") 
filledLines <- function (x, y1, y2 = rep(0, length(x)), ...) {
